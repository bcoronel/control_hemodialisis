...
class ReoporteHistorialPacienteWizard(models.TransientModel):
    ...
    def action_report_historial_paciente(self):
        ...
        fechaDesdeF = fechaDesde.strftime('%d/%m/%Y')
        fechaHastaF = fechaHasta.strftime('%d/%m/%Y') 
        
        sw_fecha = 0
        if fechaDesde != fechaHasta:
            intervalo = "DESDE " + str(fechaDesdeF) + " HASTA " + str(fechaHastaF)
            sw_fecha = 1 
        else:
            intervalo = str(fechaDesdeF)
        ...
        query = "SELECT ht.*, hp.cedula, hp.nombre, hp.conclusiones "
